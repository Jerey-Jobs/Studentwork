...
/*将fp文件中数据读出转成整形并存放在num数组中*/
int file2int(FILE *fp,char *buff,int *num)
{
    /*若读取失败打印读取错误信息*/
    fread(buff,1,MAX,fp);
...
int main(int argc,char *argv[])
{
    ...
    char src2[MAX];                   //存放文件2内容
    char r_buf[MAX];                  //存放文件3内容           
    ...
    /*打开3个文件若不存在则创建：*/
    if((fp_1 = fopen("1.txt","a+")) == NULL)
    {
        printf("open file 1 error!\n");
    }

    if((fp_2 = fopen("2.txt","a+")) == NULL)
    {
        printf("open file 2 error!\n");
    }

    if((fp_3 = fopen("3.txt","w+")) == NULL)
    {
        printf("open file 3 error!\n");
    }

    file2int(fp_1,src1,num1);               //将fp—1文件中数据读出并转换成整形到数组num1中
    file2int(fp_2,src2,num2);               //将fp—2文件中数据读出并转换到整形数组num2中
    ...
    int2str(result,fp_3);                   //将result数组内容每行4个数据存放到文件3中

    fseek(fp_3,0,SEEK_SET);                 

    while(fgets(r_buf,sizeof(r_buf),fp_3) != NULL)
    {
        printf("%s",r_buf);
