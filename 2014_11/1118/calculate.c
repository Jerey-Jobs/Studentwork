	    ...
	    /*若是运算符则判断运算符栈是否为空，若为空 直接压栈*/
	        ...
	        /*不为空 则判断优先级是否高于栈顶 若高于则压栈*/
		        ...
		        else
		        {
		            ...
		            flag2 = pop_num_stack();             //后操作数
		            flag1 = pop_num_stack();             //前操作数
                    tmp_tops = pop_symbol_stack();       //运算符

                    /*判断该运算符是哪个 并对2个操作数求值 结果压入操作数栈*/
                    if(pri(tmp_tops) == 1)
		            {
		                result = flag1 + flag2;
		            }
		            ...
		            push_num_stack(result);              //结果压入操作数栈
		            push_symbol_stack(*tmp);             //运算符入栈
...
int main()
{
    ...
    result = calculate(str);          //调用calculate函数 求出表达式的值
